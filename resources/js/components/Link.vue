<template>
    ...
    <th class="font-normal"><a :href="link.link">{{ link.link }}</a></th>
    <th class="font-normal"><a :href="link.code">{{ link.code }}</a></th>
    <th class="font-normal">
        <button @click="delete">
            <svg id="Layer_1" enable-background="new 0 0 512 512" height="20" viewBox="0 0 512 512" width="20" xmlns="http://www.w3.org/2000/svg">
...
<script>
    export default {
        ...
        methods:{
            async delete(){
                try {
                    const response = await axios.delete('/api/link/' + this.link.id)
